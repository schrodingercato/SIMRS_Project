...
export default function PendaftaranDashboard() {
  ...
  // Queue Caller state
  const [currentQueueNo, setCurrentQueueNo] = useState(24);
  const [showVerificationModal, setShowVerificationModal] = useState(false);
  const [verifyingNik, setVerifyingNik] = useState('');
  ...
  const handleVerifyDukcapil = () => {
    if (!verifyingNik) return;
    setVerificationResult(null);
    setTimeout(() => {
      setVerificationResult({
        nik: verifyingNik,
        status: 'AKTIF & VALID',
        nama: 'Siti Sarah Marlina',
        bpjs: '0002148921004 (BPJS PBI Aktif)',
        faskes: 'FKTP Puskesmas Kecamatan Gambir',
      });
    }, 600);
  };
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 space-y-6 max-w-[1600px] w-full mx-auto">
          ...
          <div className="rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.9)', boxShadow: '0 4px 20px rgba(15,23,42,0.06)' }}>
            ...
            <button onClick={() => setShowVerificationModal(true)}
              className="px-4 py-2 rounded-xl text-emerald-800 font-bold text-[0.75rem] bg-emerald-100 border border-emerald-300 hover:bg-emerald-200 transition-colors flex items-center gap-1.5">
              <span className="material-symbols-outlined text-[1rem]">verified</span>
              Verifikasi NIK Dukcapil &amp; BPJS
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7">
              <div className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.9)', boxShadow: '0 4px 20px rgba(15,23,42,0.06)' }}>
                ...
                {message && (
                  <div className={`p-3.5 rounded-xl mb-4 text-[0.8rem] font-semibold flex items-center gap-2 ${message.type === 'success' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" required maxLength={16}
                        ...
                        className="w-full px-3.5 py-2.5 rounded-xl text-[0.85rem] border border-[#bcc9c6]/50 focus:outline-none"
                  ...
                  <div className="pt-2">
                    <button type="submit" disabled={loading}
                      className="w-full py-3 px-4 rounded-xl text-white font-bold text-[0.9rem] flex items-center justify-center gap-2 shadow-lg transition-all hover:scale-[1.01]"
            ...
            {/* Loket Control Panel & Queue Caller */}
            <div className="lg:col-span-5 space-y-4">
              <div className="rounded-2xl p-5 space-y-3" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.9)', boxShadow: '0 4px 20px rgba(15,23,42,0.06)' }}>
                ...
                <div className="p-4 rounded-xl text-center text-white space-y-1" style={{ background: 'linear-gradient(135deg, #00685f, #006398)' }}>
                  ...
                  <div className="text-[3.25rem] font-black tracking-tight leading-none">A-0{currentQueueNo}</div>
                  <div className="text-[0.7rem] opacity-90">Pasien Terlayani di Loket Pendaftaran</div>
                </div>

                <div className="grid grid-cols-2 gap-2 pt-1">
                  <button onClick={() => playDingDongBell()}
                    className="py-2.5 px-3 rounded-xl font-bold text-[0.75rem] text-[#00685f] border border-[#00685f]/30 hover:bg-[#00685f]/10 transition-colors flex items-center justify-center gap-1">
                    <span className="material-symbols-outlined text-[1rem]">volume_up</span>
                    Panggil Ulang
                  </button>
                  <button onClick={handlePanggilBerikutnya}
                    className="py-2.5 px-3 rounded-xl font-bold text-[0.75rem] text-white transition-all flex items-center justify-center gap-1 shadow-md"
                    style={{ background: 'linear-gradient(90deg, #00685f, #008378)' }}>
                    <span className="material-symbols-outlined text-[1rem]">skip_next</span>
                    Berikutnya (A-0{currentQueueNo + 1})
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in zoom-in-95">
            <div className="flex items-center justify-between border-b pb-3 border-slate-100">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-emerald-600 text-[1.5rem]">verified</span>
                <h3 className="text-[1.1rem] font-extrabold text-[#131b2e]">Verifikasi NIK Dukcapil &amp; BPJS</h3>
              </div>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-[0.75rem] font-bold text-[#131b2e] mb-1">Cek NIK KTP Pasien</label>
                <div className="flex gap-2">
                  <input type="text" maxLength={16} placeholder="3171851203890061" value={verifyingNik}
                    onChange={(e) => setVerifyingNik(e.target.value)}
                    className="flex-1 px-3.5 py-2 rounded-xl text-[0.85rem] border border-slate-300 font-mono" />
                  <button onClick={handleVerifyDukcapil}
                    className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold text-[0.8rem]">
                    Cek NIK
                  </button>
                </div>
              </div>
              ...
              {verificationResult && (
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-[0.8rem] space-y-1 animate-in fade-in">
                  <div className="flex items-center justify-between font-bold text-emerald-900 border-b border-emerald-200 pb-1">
                    <span>{verificationResult.nama}</span>
                    <span className="px-2 py-0.5 rounded bg-emerald-200 text-emerald-800 text-[0.65rem]">{verificationResult.status}</span>
                  </div>
                  <div>NIK: <span className="font-mono">{verificationResult.nik}</span></div>
                  <div>No. BPJS: <span className="font-mono">{verificationResult.bpjs}</span></div>
                  <div className="text-[0.75rem] text-slate-600">{verificationResult.faskes}</div>
                </div>
              )}
            </div>
            ...
            <div className="pt-2 flex justify-end">
              <button onClick={() => setShowVerificationModal(false)}
                className="px-4 py-2 rounded-xl bg-slate-800 text-white font-bold text-[0.8rem]">
                Selesai Verifikasi
              </button>
            </div>
          </div>
